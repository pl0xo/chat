...
setlocale( LC_NUMERIC, 'C' ); //Устанавливаем разделителем "." в числах с плавающей точкой
...
define( "DBFILE", dirname( __FILE__ ) . "/chat.db" ); //Путь и имя файла с чатом
...
function makeURL( $matches ) {
	return '<a href="' . ( strpos( $matches[1], "://" ) === false ? "http://" : "" ) . $matches[1] . '">' . $matches[1] . '</a>';
}
...
if ( $mode == "list" ) {
	...
	$rlm = preg_match( "~^\\d+$~", @$_POST["lastMod"] ) ? (int)$_POST["lastMod"] : 0;
	...
	<body>
		<div id="wrapper">
			...
			<div id="msgsDialog" class="block">
				...
				<label class="autoscroll"><input id="autoScroll" type="checkbox" checked="checked" /> прокручивать до низа</label>
		...
		<script type="text/javascript">
			...
			//		XMLo.setRequestHeader( "Content-Type", "text/html; charset=windows-1251" );
